
let newImage = function (src) {
  var image = new Image()
  image.src = src
  image.onload = function() {
    this.loaded = true
  }
  return image
}

const Images = {
  "restart0": newImage('images/restart0.png'),
  "restart1": newImage('images/restart1.png'),
  "down0": newImage('images/down0.png'),
  "down1": newImage('images/down1.png'),
  "up0": newImage('images/up0.png'),
  "up1": newImage('images/up1.png'),
  "left0": newImage('images/left0.png'),
  "left1": newImage('images/left1.png'),
  "right0": newImage('images/right0.png'),
  "right1": newImage('images/right1.png'),
  "bgm_paused_true": newImage('images/bgm_paused_true.png'),
  "bgm_paused_false": newImage('images/bgm_paused_false.png'),
  "fullscreen_true": newImage('images/fullscreen_true.png'),
  "fullscreen_false": newImage('images/fullscreen_false.png'),
}
