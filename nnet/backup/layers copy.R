...
setRefClass(
  "Affine",
  fields = c("weight", "bias", "Dweight", "Dbias", "input"),
  methods = list(
  ...
  methods = list(
    forward = function(input, yTrue, oneHot = TRUE) {
      ...
      if (oneHot) {
        ...
        for (i in 1:batch) y[i, yTrue[i]] <- 1
      } 
      yTrue <<- y
...
setRefClass(
  ...
  fields = c("gamma", "beta", "momentum", "shape", 
    "Mavg", "Mvar", "xn", "xc", "std", "Dgamma", "Dbeta"
  ),
  methods = list(
...
setRefClass(
  "Convolution",
  fields = c(
    "weight", "bias", "Dweight", "Dbias", "stride", "pad", 
    "Di", "Dw", "out_h", "out_w",
    "col", "col_W"
  ),
  methods = list(
